...
两个探针均完成 4 次 run，cycle list 均为 `329;332;329;332`，总周期 1322，X 计数为 0。

## 可信度影响

这把 k3 证据从原来的 `res=1` 扩展到 `spatial_points=2` 的横向和纵向重复。它不能直接证明 `spatial=64` 的 workload 行可进主表，但能更有力地支撑 k3 projection 作为 appendix 证据，而不是完全无空间重复证据。
