...
| R5 | blocked 根因边界不够细 | high | 原先把 `attn_score` blocked 主要描述为大 `res_rows=32`，但扫描发现 `res_rows=1` 已出现 0-cycle，补测进一步显示 `cout=27/28` clean、`cout=29/30/32` blocked | 新增 `attn_score_threshold_review_v1`，并在 simulator scope 中加入 `cout>=29` 高 cout/multi-Cin 边界保护 |
