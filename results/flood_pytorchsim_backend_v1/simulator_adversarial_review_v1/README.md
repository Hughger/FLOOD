...
| R5 | blocked 根因边界不够细 | high | 原先把 `attn_score` blocked 主要描述为大 `res_rows=32`，但扫描发现 `res_rows=1` 已出现 0-cycle，补测进一步显示 `cout=27/28` clean、`cout=29/30/32` blocked，且 `cout=29` 在 `res_cols=1` clean、`res_cols>=2` blocked | 新增 `attn_score_threshold_review_v1`，并在 simulator scope 中加入 `cout>=29, res_cols>=2` 高 cout/multi-Cin 边界保护 |
...
- 因此当前 blocked 边界更接近 `cout>=29` 且 `res_cols>=2` 的高 `cout` 多列控制路径，而不是单纯大 `res_rows`。
